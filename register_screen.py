...
def register_screen():
    ...
    confirm_password_box_thickness = DESELECTED_THICKNESS
    ...
    ### DRAW SCREEN BORDERS ###
        ...
        bottom_line = Rect(0, screen_width-2, screen_height, 2) # left, top, width, height
        ...
        try:
            confirm_password = confirm_password_box.get_text()
        except NameError:
            confirm_password = ''
        ...
        password_box = InputBox(
            ...
            TEXT_FONT,
            TEXT_FONT_SIZE,
            [(screen_width/2)-INPUT_BOX_WIDTH-INPUT_BOX_HEIGHT, (screen_height/2)],
            [((screen_width/2)-INPUT_BOX_WIDTH-INPUT_BOX_HEIGHT)+TEXT_FONT_SIZE, ((screen_height/2)+INPUT_BOX_HEIGHT/2)-TEXT_FONT_SIZE/2]
        )

        confirm_password_box = InputBox(
            INPUT_BOX_WIDTH, 
            INPUT_BOX_HEIGHT,
            confirm_password,
            confirm_password_box_thickness,
            BLACK,
            screen,
            TEXT_FONT,
            TEXT_FONT_SIZE,
            [(screen_width/2)+INPUT_BOX_HEIGHT, (screen_height/2)],
            [((screen_width/2)+INPUT_BOX_HEIGHT)+TEXT_FONT_SIZE, ((screen_height/2)+INPUT_BOX_HEIGHT/2)-TEXT_FONT_SIZE/2]
        )
        ...
        show_password_button = Button(
            ...
            TEXT_FONT,
            TEXT_FONT_SIZE,
            [(screen_width/2)-SHOW_PASSWORD_WIDTH/2, screen_height/2],
            [(screen_width/2), (screen_height/2)+SHOW_PASSWORD_HEIGHT/2]
        )
        ...
        login_button = Button(
            ...
            TEXT_FONT,
            TEXT_FONT_SIZE,
            [screen_width-LOGIN_BOX_WIDTH-50, screen_height-INPUT_BOX_HEIGHT-50],
            [screen_width-LOGIN_BOX_WIDTH/2-50, (screen_height-INPUT_BOX_HEIGHT-50+TEXT_FONT_SIZE*1.5)]
        )
        ...
        confirm_password_box.init_box()
        confirm_password_box.init_side_text()
        confirm_password_box.blit_box(BLACK)
        confirm_password_box.blit_side_text(censor_on, CENSOR)
        confirm_password_box.blit_watermark(CONFIRM_PASSWORD, LIGHT_GREY)
        if confirm_password_active:
            confirm_password_box.init_flash(DESELECTED_THICKNESS)
            confirm_password_box.blit_flash(on())
    ...
    #################################
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                elif confirm_password_box.get_box().collidepoint(event.pos):
                    confirm_password_active = not confirm_password_active
                    username_active = False
                    password_active = False

                elif register_button.get_box().collidepoint(event.pos) and register_box_thickness == SELECTED_THICKNESS:
                    if register(file_to_dict(ACCOUNTS, DELIMITER), username_box.get_text()):
                        return [0, username_box.get_text(), password_box.get_text()]
        ...
        confirm_password_box_thickness = SELECTED_THICKNESS if confirm_password_active or confirm_password_box.get_box().collidepoint(pygame.mouse.get_pos()) else DESELECTED_THICKNESS
        register_box_thickness = SELECTED_THICKNESS if (register_button.get_box().collidepoint(pygame.mouse.get_pos()) and password == confirm_password and password != '' and username != '') else DESELECTED_THICKNESS
        ...
        if keys[K_ESCAPE]:
            return [2]
            # YOU CAN CONVERT THE EVENT CONSTANT, i.e K_BACKSPACE, to its charcter by chr(K_BACKSPACE)
        ...
        if username_active:
            ...
            for char in valid_characters:
                ...
                if keys[char] and char != 8 and pressed[char]:
                    ...
                    if timer(pressed_dict[chr(char)]):
                        if keys[K_RSHIFT] or keys[K_LSHIFT]:
                            confirm_password_box.add_letter(UPPER_DICT[chr(char)], round(TEXT_FONT_SIZE*9/16+TEXT_FONT_SIZE))
                        else:
                            confirm_password_box.add_letter(chr(char), round(TEXT_FONT_SIZE*9/16+TEXT_FONT_SIZE))
                    ...
                    if timer(pressed_dict[chr(char)]):
                        confirm_password_box.remove_letter()
